...
%% @type login() = #login{user=string(),secret=string(),events=string()}.
-record(login,{user,secret,events}).

%% @type logoff() = #logoff{}.
-record(logoff,{}).

%% @type mailbox_count() = #mailbox_count{mailbox=string(),new::integer(),old::integer()}.
%-record(mailbox_count,{mailbox,new,old}).

%% @type mailbox_status() = #mailbox_status{mailbox=string(),waiting::integer()}.
-record(mailbox_status,{mailbox,waiting}).
...
-record(originate,{channel,extension,context,priority,application,data,timeout,callerid,vars=[],account,async}).
...
-record(queue_status,{queue,member}).
...
%% @type_hangup() = #zap_hangup{channel=string()}.
