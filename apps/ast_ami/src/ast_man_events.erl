...
-export([init/1, handle_call/3, handle_cast/2, handle_info/2,
	 stop/0, terminate/2, code_change/3]).
...
handle_call({event,Event}, _From, State) ->
    ...
    Subs=lists:flatten([ets:lookup(State#state.subs,EType)||
			   EType <- ETypes]),
    lists:foreach(fun ({_E,Pid}) ->
			  Pid!{ast_event,Event}
		  end, lists:usort(Subs)),
    Reply = ok,
    {reply, Reply, State};

handle_call({subscribe,_User,_Secret,Pid,Events}, _From, State) ->
	_MRef=erlang:monitor(Pid),
