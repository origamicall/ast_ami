...
-author('Guillermo Rodriguez <songomem@gmail.com>').
...
new_siblings(Module) ->
	...
	SiblingEbin = filelib:wildcard(local_path(["*", "ebin"], Module)),
	Siblings = [filename:dirname(X) || X <- SiblingEbin,
		ordsets:is_element(filename:basename(filename:dirname(X)),
		Existing) =:= false],
