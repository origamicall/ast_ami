...
command(Command) ->
	case (catch gen_server:call(?SERVER,{req,#command{command=Command}},70000)) of
		{'EXIT',_} ->
			[];
		Reason ->
			Reason
...
%% @doc Delete DB Entry.
db_del(Family,Key) ->
    gen_server:call(?SERVER,{req,#db_del{family=Family,key=Key}}).
...
%mailbox_count(Mbox) ->
%    gen_server:call(?SERVER,{req,#mailbox_count{mailbox=Mbox}}).
...
%% @doc Originate Call.
originate(Channel, Exten, Context, Priority, Application, Data, Timeout, CallerId, Vars, Acct, Async) ->
    case (catch gen_server:call(?SERVER,{req,#originate{channel=Channel,
						extension=Exten,
						context=Context,
						priority=Priority,
...
handle_cast({pdu,_Msg}, State) when State#state.state==login ->
...
%%--------------------------------------------------------------------
login(User,Secret) ->
    Pdu=ast_ami_pdu:encode(#login{user=User,secret=Secret,events="off"}),
...
handle_pdu({response,_AId,Res,Pdu},State) ->
...
handle_response(undefined,_From,_Pdu,State) ->
	State;
...
handle_response({_AId,From,Cmd},Res,Pdu,State) when State#state.list_resp==false ->
   % io:format("hr1~n",[]),
...
handle_response({_AId,From,Cmd},_Res,Pdu,State) ->
