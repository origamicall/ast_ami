...
set(Loglevel) when is_integer(Loglevel) ->
   Forms = compile_string(?LOGMODULE, ast_logger_src(Loglevel)),
...
%% very efficient code
 ast_logger_src(Loglevel) ->
    ...
    %% Helper functions
    debug_msg(Module, Line, Format, Args) when " ++ L ++ " >= 5 orelse " ++ L ++ " =:= 6 ->
            notify(info_msg,
                   \"DEBUG(~p:~p:~p) : \"++Format++\"~n\",
                   [self(), Module, Line]++Args);
    debug_msg(_,_,_,_) -> ok.

    info_msg(Module, Line, Format, Args) when " ++ L ++ " >= 4 orelse " ++ L ++ " =:= 6 ->
            notify(info_msg,
                   \"INFO(~p:~p:~p) : \"++Format++\"~n\",
                   [self(), Module, Line]++Args);
    info_msg(_,_,_,_) -> ok.

    warning_msg(Module, Line, Format, Args) when " ++ L ++ " >= 3 orelse " ++ L ++ " =:= 6 ->
            notify(error,
                   \"WARNING(~p:~p:~p) : \"++Format++\"~n\",
                   [self(), Module, Line]++Args);
    warning_msg(_,_,_,_) -> ok.

    error_msg(Module, Line, Format, Args) when " ++ L ++ " >= 2 orelse " ++ L ++ " =:= 6 ->
            notify(error,
                   \"ERROR(~p:~p:~p) : \"++Format++\"~n\",
                   [self(), Module, Line]++Args);
    error_msg(_,_,_,_) -> ok.

    critical_msg(Module, Line, Format, Args) when " ++ L ++ " >= 1 orelse " ++ L ++ " =:= 6 ->
            notify(error,
                   \"CRITICAL(~p:~p:~p) : \"++Format++\"~n\",
                   [self(), Module, Line]++Args);
